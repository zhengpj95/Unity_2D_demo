...
**状态：已完成（Play Mode 验收通过）**  |  **依赖：SB-004**
...
首版参数表与资源基线见 `Docs/Modules/BalanceSystem.md`。本次只修正正式 WeaponSO 与敌人 Prefab 数值，不新增运行时覆盖配置，避免出现第二个数值真源。

验收结论：已在 Unity Play Mode 验收首版武器、玩家、敌人、Wave 与掉落数值，当前体验和数值表现符合预期。
...
SB-001、SB-002、SB-004 与 SB-005 均已完成 Play Mode 验收。下一步进入 **SB-006 UI 与视觉比例**。
