...
| `UIPresenter` | View 生命周期、界面交互与展示 | 协议收发和跨业务决策 |
...
Module 通过 `RegCmd` 将事件和 Command 绑定，框架会在模块释放时自动取消订阅。Command 不负责事件监听，只实现 `Execute`。
...
RegCmd<OpenShopCommand>("shop.open");
RegCmd<SelectShopItemCommand, int>("shop.select_item", new SelectShopItemCommand());
```
