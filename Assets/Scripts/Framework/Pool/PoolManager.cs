...
public class PoolManager : Singleton<PoolManager>
{
  ...
  public GameObject AllocUI(GameObject prefab, Transform parent)
  {
    ...
    obj.SetActive(true);
    obj.transform.SetParent(parent, false); // worldPositionStays = false，保持 UI 相对位置
  ...
  /// <summary>
  /// 回收 UI 元素（保持 Canvas 层级，不移到 PoolRoot）
  /// </summary>
  /// <param name="obj">要回收的 UI 元素</param>
  /// <param name="hide">是否隐藏对象（默认 true）</param>
  public void FreeUI(GameObject obj, bool hide = true)
  {
    if (obj == null) return;
    ...
    if (_instanceToPrefabId.TryGetValue(instanceId, out int prefabId))
    {
      ...
      // UI 元素保持在当前父节点下（Canvas），不移到 PoolRoot
      _poolDict[prefabId].Push(obj);
    }
    else
    {
